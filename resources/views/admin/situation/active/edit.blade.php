...
@section('content')
    <div class="row">
        <div class="col-md-8 offset-md-2  p-md-3 bg-dark login-box">
            <h3 class="text-info border-bottom">Edit {{$patient->name}}</h3>
            <form method="post" action="{{ route('active.update',$patient->id) }}" enctype="multipart/form-data" class="text-info mt-3">
                @method('PUT')
                @csrf
